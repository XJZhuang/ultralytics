...
@File:      2_anylabeling_json2txt.py
@Author:    123（zhuangxujun）
@Date:      2025/1/21 8:52
@Explain:   批量将X-anylabeling的 json标签 转换为 txt标签
'''
import argparse
import json
import os
...
import sys
if r"D:\1_Python\ultralytics-main\dataset_model" not in sys.path:
    sys.path.append(r"D:\1_Python\ultralytics-main\dataset_model")
    print(sys.path)
...
def main():  # noqa: C901
    ...
    parser.add_argument("--task", default="rectangle",
                        choices=["rectangle", "polygon", "rotation", "mots"], help="Choose the type of task to perform",)
    parser.add_argument('--group_list', nargs='+', type=str, default=[])
    parser.add_argument("--src_path", default=r"D:\1_Python\datasets\1_detect_electricity_yantian\labels_json\train_",
                        help="Path to input file or directory。anything JSON格式标签的根路径")
    parser.add_argument("--dst_path", default=r"D:\1_Python\datasets\1_detect_electricity_yantian\labels\train_",
                        help="Path to output file or directory.转换后txt标签的保存路径")
    parser.add_argument("--img_path", default=r"D:\1_Python\datasets\1_detect_electricity_yantian\images\train_",
                        help="Path to image file or directory.图片路径")
    parser.add_argument("--classes",
                        # default=r"D:\1_Python\datasets\0_detect_helmet\classes.txt",
                        default=r"D:\1_Python\datasets\classes.txt",
                        help="Path to classes.txt file, where each line represent a specific class", )
    parser.add_argument(
        "--mode", default="custom2yolo", help="Choose the conversion mode what you need",
        choices=["custom2voc", "voc2custom", "custom2yolo", "yolo2custom", "custom2coco", "coco2custom",
                 "custom2dota", "dota2custom", "dota2dcoco", "dcoco2dota", "dxml2dota", "custom_to_gt", ],)
    args = parser.parse_args()

    # src_path0 = args.src_path
    # dst_path0 = args.dst_path
    # img_path0 = args.img_path

    # 1.anything JSON格式标签的根路径
    src_path0 = r"D:\1_Python\datasets\4_detect_electricity_baoan\labels_json\train_"
    # src_path0 = r"D:\1_Python\datasets\0_detect_helmet\labels_json\train_"
    # 2.转换后txt标签的保存路径
    dst_path0 = r"D:\1_Python\datasets\4_detect_electricity_baoan\labels\train_"
    # dst_path0 = r"D:\1_Python\datasets\0_detect_helmet\labels\train_"
    # 3.图片路径
    img_path0 = r"D:\1_Python\datasets\4_detect_electricity_baoan\images\train_"
    # img_path0 = r"D:\1_Python\datasets\0_detect_helmet\images\train_"

    # 遍历一个区分的所有班组
    # group_index = ['yant_dh', 'helmet']   # 盐田东湖，安全帽
    # group_index = ['dh', 'lt', 'stj', 'ytg']   # 盐田
    # group_index = ['hg', 'hmg', 'lhb', 'ml', 'st', 'yt', 'zzl']   # 福田
    group_index = ['fh', 'fy', 'gs', 'sg', 'sj', 'sy', 'xa', 'xq', 'xx', 'ya']   # 宝安
    # group_index = ['hb', 'lh', 'ng', 'tb']   # 罗湖
    # group_index = ['ytfh', 'gmmt']   # 光明
    # group_index = ['dl', 'gl', 'mz']   # 龙华
    # group_index = ['dp', 'kc', 'kz', 'lt', 'ml', 'sj']   # 坪大
    # group_index = ['btxg', 'btzx', 'bjzx', 'hghg', 'hgly', 'hgsl', 'lcal', 'lclz', 'lcsp', 'lgjx', 'lglj', 'lgtl', 'nwbl', 'nwsw', 'phpx', 'pdzx']  # 龙岗
    # group_index = ['cs', 'eb', 'hm', 'xm']  # 深汕
    # group_index = args.group_list
    for group in group_index:
        args.src_path = src_path0 + group
        args.dst_path = dst_path0 + group
        args.img_path = img_path0 + group
        print(args.src_path)
        print(args.dst_path)
        print(args.img_path)
        ...
        if args.task == "rectangle":
            ...
            valid_modes = ["custom2voc", "voc2custom", "custom2yolo", "yolo2custom", "custom2coco", "coco2custom", ]
            assert (args.mode in valid_modes), f"Rectangle tasks are only supported in {valid_modes} now!"
        else:
            raise Exception

        if args.mode == "custom2yolo":
