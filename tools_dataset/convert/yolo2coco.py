...
def yolo2coco(yolo_dataset_path, output_json_path, subset_file=None):
    ...
    if not os.path.exists(classes_file):
        LOGGER.warning(f"未找到 {classes_file} 文件，尝试在其他目录中查找classes.txt")
    ...
    for i, cls_name in enumerate(classes):
        coco_data["categories"].append({
            "id": i + 1,  # 修改点1：类别ID从1开始（COCO格式）
            "name": cls_name,
            "supercategory": "switchgear_component"
        })
    ...
    for img_id, img_path in enumerate(tqdm(image_files, desc="处理图像")):
        ...
        try:
            ...
            for line in label_lines:
                ...
                # 获取类别名称和新的类别ID
                cls_name = classes[cls_id]      # classes.txt中的类别名称
...
def main():
    ...
    parser.add_argument("--dataset_path", type=str, default=r"D:\1_Python\datasets\switchgear", help="YOLO数据集的根目录")
    parser.add_argument("--output", type=str, default=r"D:\1_Python\datasets\switchgear\labels\instances_train.json", help="输出的COCO格式JSON文件路径")
    parser.add_argument("--subset", type=str, default=r"D:\1_Python\datasets\switchgear\labels\train.txt", help="可选，指定包含要处理的图像列表的文本文件路径")
